...
findArriveTime2 = re.compile(
    r'class="airport arrive".*class="airport-time".*><span.*>(.*)</span><span class="airport-time-days".*>.*')
# 到达机场
...
def writeData(infolist, datalist, savepath):
    ...
    if table.nrows == 1:
        col = (
        "id", "departure", "destination", "date", "company", "model", "leaveTime", "leavePort", "way", "arriveTime",
